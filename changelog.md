...
**The assembly this pipeline produces changes in this release.** The bundled `orp-transrate` (Ruby transrate 1.0.3, pinned to Ruby 2.2.0, x86_64-linux-only, depending on three abandoned binaries) is replaced by [pytransrate](https://github.com/macmanes-lab/pytransrate) 2.0.0, a Python reimplementation targeting current versions of the same dependencies. pytransrate computes the four score components, the contig score and the assembly score from the same formulas, but its inputs changed -- snap-aligner 1.0dev.96 -> 2.0.5 (whose soft clipping the old `bam-read` helper mishandled, shifting a clipped read's coverage contribution rightward), salmon 0.8.2 -> 2.7.0, and a redesigned fragment-assignment step, since salmon 0.8.2's own posterior sampling is not recoverable from outside. Measured against the Ruby on three assemblies of one library: the assembly score drops 0.008-0.070 and 14.5-19% of contig pairs are ordered oppositely. Because `makeorthout` keeps the highest-scoring member of each orthogroup, that reordering reaches the assembly itself, not just the reported number -- so the `TRANSRATE SCORE` lines in `reports/qualreport.<run>` are not comparable to any earlier ORP release's. pytransrate's `CHANGELOG.md` records what changed and by how much; its `scripts/compare_orthogroup_picks.py` measures how far the orthogroup picks actually move between two runs.

- replace the bundled `software/orp-transrate.tar.gz` (32MB, unpacked by `make` and added to `PATH`) with pytransrate installed from its git tag by `orp_env.yml`. The `transrate` Makefile target, its `PATH` entry in `pathfile` and in the Dockerfile, and the manual unpack/PATH steps in INSTALL.md all go away; nothing needs to be on `PATH` outside the `orp` env any more
- bump `salmon=2.5.1` -> `2.7.0` in `orp_env.yml`. This is required, not cosmetic: pytransrate passes salmon's `--errorModel` on every run, and that flag only exists from salmon 2.6 on (it was `--useErrorModel` before). It matters because salmon 2.6 also made alignment-mode scoring read the BAM `AS` tag, which snap-aligner does not emit -- without `--errorModel`, multireads get apportioned by effective length alone, which is materially worse on a de-novo transcriptome full of near-duplicate contigs. ORP's own salmon quant steps move to 2.7.0 along with it, since they share the `orp` env
- add `snap-aligner=2.0.5` and `pysam` to `orp_env.yml`: pytransrate's two other runtime requirements. snap-aligner is new to ORP entirely (the Ruby bundle shipped its own copy); `check()` now verifies both it and `pytransrate` in the preflight pass, replacing the executable-path test against the unpacked bundle
- delete the output directory before each of `orthotransrate()`/`transrate()`. pytransrate refuses an output directory that already holds an `assemblies.csv` rather than silently overwriting it, and `needs_run()` re-runs either step whenever the corrected reads are newer than the csv -- not only when the csv is absent -- so a resumed run would otherwise abort where the Ruby just overwrote
- drop the `rglob("*.bam")` cleanup loops from both steps: pytransrate deletes its own BAM on success unless `--keep-bam` is passed
