...
- fix `STEP_TIME_HINTS`, the reference profile `run_parallel()` uses to submit the slower job in a concurrent pair first: real timing data showed `merge_branch` and `transrate` were being submitted *after* their faster sibling (`orthofuser_branch`, `strandeval`) despite being the slower job, defeating the point of longest-first ordering. Also dropped two entries (`run_orthofuser`, `orthotransrate`) that were never actually looked up, since `run_parallel()` only keys on the name passed to it, not a branch function's internal sub-steps
