...
| `makelist` | pure Python | `Orthogroups.txt` | `orthofuse/<run>/<run>.list` | Just an index `1..n` (n = orthogroup count) — feeds the range `makegroups` iterates. |
| `makegroups` | pure Python, thread pool | `Orthogroups.txt`, the index list | `orthofuse/<run>/<i>.groups` ×n; `groups.done` | Splits `Orthogroups.txt` into one file per orthogroup, each listing its member contig IDs. |
...
- **The bookkeeping chain (`make_list1`–`make_list7`, `diamond_uniq`, `orp_uniq`, `filter`, `merge`) is about as cheap as it can get** — pure single-pass Python, no subprocess overhead, sub-second on real data (see the timing tables in [benchmarks.md](../sampledata/benchmarks.md)). Not a place to spend further optimization effort.
- **`makegroups` does not belong in that list, and was never measured.** It writes one `<i>.groups` file per orthogroup — of order 10⁵ of them into a single directory — which `pick_best_contigs.py` globs back in and `makeorthout` then unlinks: three metadata-heavy passes over files that are written by Python, read by Python and deleted by Python, and only ever cross a process boundary because the picker runs through `conda run` (it imports nothing outside the stdlib, so it doesn't actually need the `orp` env). `makeorthout` is the only one of the three passes with a timing (49s–1m33s on real runs), so it bounds the read side; the write and delete sides were `timed=False` and are now timed so the next real run reports them. **Anything that replaces this round-trip has to preserve `good.<run>.list` ordering**: the picker iterates `sorted(glob.glob("*groups"))`, which is lexicographic on filenames (`1, 10, 100, 2, …`), and that order reaches contig order in `orthomerged.fasta` and so into `cd-hit-est`, where it breaks length ties and can change which representative survives into `.ORP.fasta`. Emitting in `sorted(str(i) for i in range(1, n + 1))` order keeps the list byte-identical; iterating `Orthogroups.txt` in file order does not, and would make a pure performance cleanup an output-changing release.
- **`makelist`'s output is never read.** `orthofuse/<run>/<run>.list` is written, declared as `makegroups`'s input, and never opened — `makegroups` re-reads `Orthogroups.txt` itself and iterates `range(1, len(lines) + 1)`. Left in place for now because it also sequences the two steps, but it goes with any rewrite of the round-trip above.
