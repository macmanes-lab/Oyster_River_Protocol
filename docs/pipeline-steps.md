...
- **The `.groups` round-trip is gone (was: 10⁵ files written, globbed back in, unlinked).** `makegroups` used to split `Orthogroups.txt` into one `<i>.groups` file per orthogroup, `pick_best_contigs.py` globbed them all back, and `makeorthout` then unlinked every one — three metadata-heavy passes over files written by Python, read by Python and deleted by Python, that only ever crossed a process boundary because the picker runs through `conda run` (it imports nothing outside the stdlib). The picker now takes `Orthogroups.txt` directly and `makelist`/`makegroups` are gone, along with `makelist`'s `<run>.list`, which was written, declared as `makegroups`'s input, and never actually opened. **Group ordering is preserved exactly**, and that was the whole risk: the old glob sorted *filenames*, which is lexicographic (`1, 10, 100, 2, …`) rather than numeric, and that order reaches contig order in `orthomerged.fasta` and so `cd-hit-est`, where it breaks length ties and can change which representative survives into `.ORP.fasta`. The sort key inside the picker reproduces it; `good.<run>.list` is byte-identical across the change on synthetic orthogroup sets from n=1 to n=1111 covering ties, zero and negative scores, contigs missing from `contigs.csv`, duplicate contig rows and blank lines. pytransrate's `compare_orthogroup_picks.py` was never affected — it already read `Orthogroups.txt` via `--orthogroups` and rebuilt the groups in memory.
