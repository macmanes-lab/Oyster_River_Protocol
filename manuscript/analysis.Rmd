...
detonbean <- function(titles)
{
  ...
  beanplot(matrix$value ~ matrix$Var2, 
           side="both",col = list("sienna4", "dodgerblue"), ll = 0, beanlinewd=0,
           main=do.call(sprintf, as.list(c(title, titles))),
           boxwex=1, xlim=c(1.1,1.9), at=c(1.5), frame.plot = F,  ylab='Score', axes=F)
  Corner_text(text="C")
  axis(2)
  axis(1, at=c(1.3, 1.7),  labels=c("Trinity-Only","ORP"))
  n <- beeswarm(matrix$value ~ matrix$Var2,
...
scorebean <- function(titles)
{
  ...
  beanplot(matrix$value ~ matrix$Var2, 
           side="both",col = list("sienna4", "dodgerblue"), ll = 'F', beanlinewd=0,
           main=do.call(sprintf, as.list(c(title, titles))),
...
crbbean <- function(titles)
{
  ...
  beanplot(matrix$value ~ matrix$Var2, 
           side="both",col = list("sienna4", "dodgerblue"), ll = 'F', beanlinewd=0,
           main=do.call(sprintf, as.list(c(title, titles))),
           boxwex=1, xlim=c(1.1,1.9), at=c(1.5), frame.plot = F, ylim=c(3000,20000), ylab='Number of hits', axes=F)
  Corner_text(text="A")
  axis(2)
  #axis(1, at=c(1.3, 1.7),  labels=c("Trinity-Only","ORP"))
  n <- beeswarm(matrix$value ~ matrix$Var2,
...
percentbean <- function(titles)
{
  ...
  beanplot(orp$percentS/orp$contigNumO,  ll = 'F', beanlinewd=0, boxwex=1, xlim=c(1.1,1.9), at=c(1.5), frame.plot = F, ylim=c(0,.6), ylab='Percent Contribution', axes=T, col="blue")
  axis(1, at=c(1.5),  cex=1.3,labels=c("Shannon"),tick="F")
  axis(2, at=c(.3), cex=1.3,labels=c("Percent Contribution"), outer = "T", tick="F")
  beanplot(orp$percent55/orp$contigNumO,  ll = 'F', beanlinewd=0, boxwex=1, xlim=c(1.1,1.9), at=c(1.5), frame.plot = F, ylim=c(0,.6), axes=F, col="black")
  axis(1, at=c(1.5),  labels=c("SPAdes55"), tick="F")
    beanplot(orp$percent75/orp$contigNumO,  ll = 'F', beanlinewd=0, boxwex=1, xlim=c(1.1,1.9), at=c(1.5), frame.plot = F, ylim=c(0,.6), axes=F, col="red")
    axis(1, at=c(1.5),  labels=c("SPAdes75"), tick="F")
      beanplot(orp$percentT/orp$contigNumO,  ll = 'F', beanlinewd=0, boxwex=1, xlim=c(1.1,1.9), at=c(1.5), frame.plot = F, ylim=c(0,.6), axes=F, col="green")
      axis(1, at=c(1.5),  labels=c("Trinity"), tick="F")
}
...
png("/Volumes/4TB_1/Dropbox/feeding/Fig2.png", width = 1500, height = 3000, units = 'px', res = 300)
par(mfrow = c(3,1),
oma = c(0,0,0,0) + 0.1,
mar = c(0,0,1,1) + 0.1,
bty = 'n',
...
png("/Volumes/4TB_1/Dropbox/feeding/Fig3.png", width = 900, height = 2000, units = 'px', res = 300)
par(mfrow = c(3,1),
mar = c(0,3,1,1) + 0.1,
oma = c(0,0,0,0) + 0.1,
bty = 'n',
...
```{r percentbean, echo=FALSE}
png("/Volumes/4TB_1/Dropbox/feeding/Fig4.png", width = 3600, height = 2700, units = 'px', res = 300)
percentbean("Percent Sequence Contribution")
dev.off()
```
...
DRR030368_ggjoy <- read_csv("/Volumes/4TB_1/Dropbox/orpstats/DRR030368.ggjoy.sf")
ERR058009_ggjoy <- read_csv("/Volumes/4TB_1/Dropbox/orpstats/ERR058009.ggjoy.sf")
DRR053698_ggjoy <- read_csv("/Volumes/4TB_1/Dropbox/orpstats/DRR053698.ggjoy.sf")
DRR082659_ggjoy <- read_csv("/Volumes/4TB_1/Dropbox/orpstats/DRR082659.ggjoy.sf")

png("/Volumes/4TB_1/Dropbox/feeding/Fig5.png", width = 3600, height = 2700, units = 'px', res = 300)

plot1 <- ggplot(DRR053698_ggjoy, aes(x = log(tpm+1), y = assembler, fill = assembler)) + geom_density_ridges(scale = 0.9) + xlim(0,5) + scale_fill_cyclical(values = c("blue", "black", 'red', 'green')) + ggtitle("DRR053698") + theme_ridges(grid = FALSE, center_axis_labels = TRUE) + theme(axis.title.y=element_blank(),axis.text.y=element_blank(),axis.ticks.y=element_blank())

plot2 <- ggplot(ERR058009_ggjoy, aes(x = log(tpm+1), y = assembler, fill = assembler)) + geom_density_ridges(scale = 0.9) + xlim(0,5) + scale_fill_cyclical(values = c("blue", "black", 'red', 'green'))+ ggtitle("ERR058009") + theme_ridges(grid = FALSE, center_axis_labels = TRUE) + theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank()) + theme(axis.title.y=element_blank(),axis.text.y=element_blank(),axis.ticks.y=element_blank())

plot3 <- ggplot(DRR030368_ggjoy, aes(x = log(tpm+1), y = assembler, fill = assembler)) + geom_density_ridges(scale = 0.9) + xlim(0,5) + scale_fill_cyclical(values = c("blue", "black", 'red', 'green'))+ ggtitle("DRR030368") + theme_ridges(grid = FALSE, center_axis_labels = TRUE)

plot4 <- ggplot(DRR082659_ggjoy, aes(x = log(tpm+1), y = assembler, fill = assembler)) + geom_density_ridges(scale = 0.9) + xlim(0,5) + scale_fill_cyclical(values = c("blue", "black", 'red', 'green'))+ ggtitle("DRR082659") + theme_ridges(grid = FALSE, center_axis_labels = TRUE) + theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank())
...
beanplot(orp_map ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'ORP Mapping', ylab='%Reads Mapping', ylim=c(.5, 1))
beanplot(trin_map ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'Trinity Mapping', ylab='', ylim=c(.5, 1))

beanplot(orp_score ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'ORP Score', ylab='Score', ylim=c(0, .5))
beanplot(trin_score ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'Trinity Score', ylab='', ylim=c(0, .5))


beanplot(orp_complete ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'ORP Complete', ylab='% Complete', ylim=c(0, 100))
beanplot(trin_complete ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'Trinity Complete', ylab='', ylim=c(0, 100))

beanplot(orp_duplicate ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'ORP Duplicated', ylab='% Duplication', ylim=c(0, 100))
beanplot(trin_duplicate ~ taxa,  ll = 'F', beanlinewd=0, col = list("yellow", "green", "orange"), main = 'Trinity Duplicated', ylab='', ylim=c(0, 100))
...
png("~/Dropbox/feeding/Fig6.png", width = 3600, height = 3600, units = 'px', res = 300)
...
```{r mash nmds}
DRR030368_mash <- read_csv("~/Oyster_River_Protocol/manuscript/DRR030368.mash.csv")
DRR031870_mash <- read_csv("~/Oyster_River_Protocol/manuscript/DRR031870.mash.csv")
DRR036858_mash <- read_csv("~/Oyster_River_Protocol/manuscript/DRR036858.mash.csv")
DRR046632_mash <- read_csv("~/Oyster_River_Protocol/manuscript/DRR046632.mash.csv")
DRR053698_mash <- read_csv("~/Oyster_River_Protocol/manuscript/DRR053698.mash.csv")
...
png("/Volumes/4TB_1/Dropbox/feeding/Fig1.png", width = 3600, height = 2700, units = 'px', res = 300)
plot(isoMDS(dist(DRR031870_mash))$points, frame.plot = F, col=c("red", "green", "blue", "orange", "black"), pch=4, lwd=4, ylim=c(-1, .8), xlim=c(-1.5, .7), ylab="MDS Parameter 2", xlab="MDS Parameter 1")
...
legend(-1.6, -.5,  col=c("red", "green", "blue", "orange", "black"), legend=c('Orthomerged', 'Shannon', 'SPAdes55', 'SPAdes75', 'Trinity'), bty='n', pch=4)
dev.off()
```
