...
%DIF DEL old.tex                     Fri May 11 10:07:32 2018
%DIF ADD feeding_peerj_revised.tex   Fri May 11 09:59:01 2018
...
%DIF 22c22
%DIF < \usepackage[style=authoryear, minbibnames=99, maxbibnames=9, maxcitenames=9, natbib=true, backend=biber]{biblatex}
%DIF -------
\usepackage[style=authoryear, minbibnames=9, maxbibnames=99, maxcitenames=9, natbib=true, backend=biber]{biblatex} %DIF > 
%DIF -------
...
\\
Matthew D. MacManes$^{1,}$ \DIFaddbegin \DIFadd{$^{2,}$ }\DIFaddend $^{\ast,}$ $^{\bullet,}$ $^\star$

$^{1}$ Department of Molecular, Cellular and Biomedical Sciences, University of New Hampshire, Durham NH, USA
\DIFaddbegin \DIFadd{$^{2}$ Hubbard Center For Genomic Studies
}\DIFaddend 
...
$\star$ Mailing Address: \DIFdelbegin \DIFdel{46 College Road, 189 Rudman }\DIFdelend \DIFaddbegin \DIFadd{35 Colovos Road, 434 Gregg }\DIFaddend Hall. Durham NH 03824
\end{flushleft}
...
In addition to the variety of tools available for the \tit{de novo} assembly of transcripts, several tools are available for pre-processing of reads via read trimming ((\eg\ \texttt{Skewer} \citep{Jiang:2014cx}, \texttt{Trimmomatic} \citep{Bolger:2014ek}, \texttt{Cutadapt} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Martin:2011va}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Martin:2011va}}%DIFAUXCMD
}\DIFaddend ), read normalization (\texttt{khmer} \citep{Pell:2012id}), and read error correction (\texttt{SEECER} \citep{Le:2013dy} and \texttt{RCorrector} \citep{Song:2015in}, \texttt{Reptile} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Yang:2010kv}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Yang:2010kv}}%DIFAUXCMD
...
For all assemblies performed, Illumina sequencing adapters were removed from both ends of the sequencing reads, as were nucleotides with quality Phred $\leq$ 2, using the program \texttt{Trimmomatic} version 0.36 \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Bolger:2014ek}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Bolger:2014ek}}%DIFAUXCMD
}\DIFaddend , following the recommendations from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{MacManes:2014io}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{MacManes:2014io}}%DIFAUXCMD
}\DIFaddend . After trimming, reads were error corrected using the software \texttt{RCorrector} version 1.0.2 \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Song:2015in}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Song:2015in}}%DIFAUXCMD
}\DIFaddend , following recommendations from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{MacManes:2013ec}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{MacManes:2013ec}}%DIFAUXCMD
...
All assemblies were evaluated using \texttt{ORP-TransRate}, \texttt{Detonate} version 1.11 \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li:2014cm}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Li:2014cm}}%DIFAUXCMD
}\DIFaddend , \texttt{shmlast} version 1.2 \citep{Scott:2017eg}, and \texttt{BUSCO} version 3.0.2 \citep{Simao:2015kk}. \texttt{TransRate} evaluates transcriptome assembly contiguity by producing a score based on length-based and mapping metrics, while \texttt{Detonate} conducts an orthogonal analysis, producing a score that is maximized by an assembly that is representative of input sequence read data. \texttt{BUSCO} evaluates assembly content by searching the assemblies for conserved single copy orthologs found in all Eukaryotes. I report default \texttt{BUSCO} metrics as described in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Simao:2015kk}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Simao:2015kk}}%DIFAUXCMD
...
Of note, consistent with all short-read assemblers \citep{Ungaro:2017kf}, the ORP assemblies may not accurately reflect the true isoform complexity. Specifically, because of the way that single representative transcripts are chosen from a cluster of related sequences, some transcriptional complexity may be lost. Consider the cluster containing contigs \{AB, A, B\} where AB is a false-chimera, selecting a single representative transcript with the best score could yield either A or B, thereby excluding an important transcript in the final output. I believe this type of transcript loss is not common, based on how contigs are scored (Table 1, Figure 3, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{SmithUnna:2016go}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{SmithUnna:2016go}}%DIFAUXCMD
...
\DIFdelbegin %DIFDELCMD < \centerline{\includegraphics[width=25.0\baselineskip]{Fig2.png}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \centerline{\includegraphics[width=13.0\baselineskip]{Fig2.png}}
...
\DIFdelbegin %DIFDELCMD < \centerline{\includegraphics[width=25.0\baselineskip]{Fig3.png}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \centerline{\includegraphics[width=15.0\baselineskip]{Fig3.png}}
\DIFaddend \begin{quote}
...
\begin{quote}
\DIFdelbegin %DIFDELCMD < \small{Figure 4 describes the percent contribution of each assembler to the final ORP assembly.}
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdel{For non-model organisms lacking reference genomic resources, the error corrected, adapter- and quality-trimmed reads must be assembled }\textit{\DIFdel{de novo}} %DIFAUXCMD
\DIFdel{into transcripts. While the assembly package }\texttt{\DIFdel{Trinity}} %DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\citep{Haas:2013jq} }%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{Specifically, the improvements in assembly metrics described here are attributed to the multi-way approach, where three different assemblers and three different kmer lengths were used. This approach allows the strengths of one assembler to effectively complement the weaknesses of another, thereby resulting in a more complete assembly than otherwise possible. These enhancements are important, as unassembled transcripts are invisible to all downstream analysis.      
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section*{Acknowledgments}

This work was significantly improved by discussions with \DIFaddbegin \DIFadd{Anthony Westbrook, }\DIFaddend Richard Smith-Unna \DIFdelbegin \DIFdel{, Rob Patro , }\DIFdelend \DIFaddbegin \DIFadd{and Rob Patro and reviewers }\DIFaddend C. Titus Brown, Brian Haas\DIFdelbegin \DIFdel{and many others. More generally, the work and its presentation has been influenced by supporters of the Open Access and Open Science movements}\DIFdelend \DIFaddbegin \DIFadd{, and Nick Schurch. MacManes is supported by the NSF (IOS 1455960, OCE 1455960, DEB 1655585), XSEDE (MCB110134), and the UNH Research and Computing Center}\DIFaddend . 
