...
orthofusing:
	...
	ls ${DIR}/orthofuse/${DATASET}.${SAMP}/*txt | parallel -j $(CPU) "grep -wf {} $$(find ${DIR}/orthofuse/${DATASET}.${SAMP}/ -name contigs.csv 2> /dev/null) > ${DIR}/orthofuse/${DATASET}.${SAMP}/{1}.out" && \
	find ${DIR}/orthofuse/${DATASET}.${SAMP}/ -name *txt -delete && \
	ls ${DIR}/orthofuse/${DATASET}.${SAMP}/	*out | parallel -j $(CPU) "awk -F, 'BEGIN {max = 0} {if (\$$9>max) max=\$$9} END {print \$$1 \"\\t\" max}'" | tee -a ${DIR}/orthofuse/${DATASET}.${SAMP}/good.list && \
